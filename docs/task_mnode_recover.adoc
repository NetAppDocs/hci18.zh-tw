...
.您需要的產品
* 您先前的管理節點執行的是 NetApp Element 軟體 11.3 版或更新版本、且link:concept_hci_volumes.html#persistent-volumes["持續磁碟區"]已啟用功能。
...
NOTE: 從元件12.3.1開始、會自動執行子步驟（A）至（e）。對於管理節點 12.3.1 、請繼續執行<<substep_f_recover_config_time_sync,子步驟（f）>>以完成時間同步組態。
...
.. 為您要新增的每個預設時間伺服器新增一行。預設時間伺服器必須與您要在中使用的儲存叢集上所使用的 NTP 伺服器相同link:task_mnode_recover.html#configure-the-management-node["後續步驟"]。
+
[listing]
----
vi /etc/ntp.conf
...
NOTE: 完成時間同步組態後、請勿編輯 NTP 、因為當您在管理節點上執行時、 NTP 會受到影響<<step_6_recover_mnode_redeploy,重新部署命令>>。
...
----
. 下載先前安裝在現有管理節點上的管理服務套件（版本 2.15.28 或更新版本）、並將其儲存在目錄中 `/sf/etc/mnode/`。
...
----
. 將產生的檔案解壓縮至 `/sf/etc/mnode-archive`目錄：
+
[listing]
----
...
+
*** *[mvip IP位址]*：儲存叢集的管理虛擬IP位址。使用您在中使用的相同儲存叢集link:task_mnode_recover.html#configure-time-sync["NTP伺服器組態"]來設定管理節點。
...
.. 執行 `redeploy-mnode`命令。重新部署完成時、指令碼會顯示成功訊息。
.. 如果您使用系統的完整網域名稱（ FQDN ）存取元素或 NetApp HCI 網路介面（例如管理節點或 NetApp 混合雲控制）、link:task_hcc_upgrade_management_node.html#reconfigure-authentication-using-the-management-node-rest-api["重新設定管理節點的驗證"]。
