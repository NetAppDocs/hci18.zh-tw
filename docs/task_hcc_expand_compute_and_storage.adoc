...
. 在 * 歡迎 * 頁面上、選取 * 是 * 、然後選取 * 繼續 * 。
. 在 * 終端使用者授權 * 頁面上、閱讀 VMware 終端使用者授權合約、然後選取 * 我接受 * 以接受條款、然後選取 * 繼續 * 。
. 在* vCenter*頁面上、完成下列步驟：
+
.. 輸入與NetApp HCI 您的安裝有關的vCenter執行個體的FQDN或IP位址和管理員認證。
.. 選擇*繼續*。
.. 選取您要新增運算節點的 vSphere 資料中心、或選取 * 建立新的資料中心 * 、將運算節點新增至新的資料中心。
+
...
NOTE: 如果NetApp HCI 無法辨識您選擇要擴充的叢集網路設定、請確定管理、儲存設備和VMotion網路的vmkernel和vmnic對應已設定為部署預設值。如需詳細資訊、請參閱 link:task_nde_supported_net_changes.html["支援的網路變更"] 。
...
. 在 * 可用庫存 * 頁面上、選取您要新增的儲存和運算節點、然後選取 * 繼續 * 。
+
